...
def simulate(modifications=None, count=1, failure=None, wrong_type=False):
    stock=(ROOT/'firmwares/hw501/131/rootfs/bin/CPAAProxyEx').read_bytes()
    patched,meta=patch(stock)
...
class DisplayPatchTests(unittest.TestCase):
    def test_only_physical_dimensions_change(self):
        result,logs,_=simulate()
        self.assertEqual(result,{'widthPixels':800,'heightPixels':480,'widthPhysical':190,'heightPhysical':114,'maxFPS':60,'uuid':'unchanged','features':8,'touch':'unchanged'})
        self.assertEqual(len(logs),1)
        self.assertIn('190x114mm',logs[0])
    ...
    def test_other_displays_are_unchanged(self):
        for key,value in [('widthPixels',1024),('heightPixels',600),('widthPhysical',160),('heightPhysical',90),('widthPixels',(1<<32)+800)]:
            with self.subTest(key=key,value=value):
                result,logs,_=simulate({key:value});self.assertIsNone(result);self.assertEqual(logs,[])

    def test_multiple_displays_unchanged(self):
        self.assertIsNone(simulate(count=2)[0])

    def test_wrong_dictionary_type_unchanged(self):
        self.assertIsNone(simulate(wrong_type=True)[0])
    ...
    def test_allocation_and_number_failures_fall_back_without_leaks(self):
        for failure in ('copy','widthPhysical','heightPhysical','array'):
            with self.subTest(failure=failure):self.assertIsNone(simulate(failure=failure)[0])

    def test_wrong_binary_rejected(self):
        with self.assertRaises(ValueError):patch(b'wrong firmware')
